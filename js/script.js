...
function findCircle(graph, station) {
    ...
    var stationPlatforms = station.platforms.map(function (platformNum) { return graph.platforms[platformNum]; });
    if (stationPlatforms.length === 3) {
        return stationPlatforms.every(function (p) { return p.transfers.length === 2; }) ? stationPlatforms : [];
    }
    if (stationPlatforms.length === 4) {
        var ps = stationPlatforms.slice().sort(function (a, b) { return a.transfers.length - b.transfers.length; });
        var degs = ps.map(function (p) { return p.transfers.length; });
...
function shortestRoute(graph, p1, p2) {
    ...
    var distanceBetweenPoints = function (a, b) { return L.LatLng.prototype.distanceTo.call(a, b); };
    var objects = graph.platforms;
    // time to travel from station to the p1 location
    var dist = [], timesOnFoot = [];
    for (var _i = 0; _i < objects.length; _i++) {
        var o = objects[_i];
        var hasE = o.spans.map(function (i) { return graph.spans[i].routes[0]; })
            .map(function (i) { return graph.routes[i].line; })
            .some(function (l) { return l.startsWith('E'); });
        var distance = distanceBetweenPoints(p1, o.location), time = distance / walkingWithObstacles;
    ...
    var onFoot = distanceBetweenPoints(p1, p2) / walkingWithObstacles;
    while (objectSet.size > 0) {
        ...
        var prevSpan = graph.spans.find(function (s) { return s.source === currentIndex && s.target === prevIndex || s.source === prevIndex && s.target === currentIndex; });
        var neighborIndices = [], times = [];
        for (var _a = 0, _b = currentNode.spans; _a < _b.length; _a++) {
            ...
            var distance = distanceBetweenPoints(currentNode.location, neighbor.location);
            var lineChangePenalty = 0;
            if (prevSpan && graph.routes[prevSpan.routes[0]] !== graph.routes[s.routes[0]]) {
            }
            // TODO: lower priority for E-lines
            var callTime = graph.routes[s.routes[0]].line.startsWith('E') ? eLineStopTime : metroStopTime;
        ...
        for (var _c = 0, _d = graph.stations[currentNode.station].platforms; _c < _d.length; _c++) {
            ...
            var hasE = neighbor.spans
                .map(function (i) { return graph.spans[i].routes[0]; })
                .map(function (i) { return graph.routes[i].line; })
                .some(function (l) { return l.startsWith('E'); });
            var distance = distanceBetweenPoints(currentNode.location, neighbor.location) / 2;
        ...
        for (var i = 0, nNeighbors = neighborIndices.length; i < nNeighbors; ++i) {
            var neighborIndex = neighborIndices[i], alt_1 = dist[currentIndex] + times[i];
            if (alt_1 < dist[neighborIndex]) {
                dist[neighborIndex] = alt_1;
                prev[neighborIndex] = currentIndex;
            }
        }
        var alt = dist[currentIndex] + timesOnFoot[currentIndex];
    }
    // find the shortest time & the exit station
    var shortestTime = Infinity;
    for (var i = 0; i < dist.length; ++i) {
        var alt = dist[i] + timesOnFoot[i];
        if (alt < shortestTime) {
            shortestTime = alt;
            currentIndex = i;
        }
    ...
    for (;;) {
        ...
        if (p === '') {
            for (var i = 0, len = graph.transfers.length; i < len; ++i) {
                var t = graph.transfers[i];
        ...
        if (p === '') {
            var _g = shortestTransfer(graph, prevIndex_1, currentIndex), transfers = _g.transfers, midPlatforms = _g.midPlatforms;
    ...
    path.reverse();
    var walkFromTime = timesOnFoot[platformPath[platformPath.length - 1]];
    var walkToTime = dist[platformPath[0]];
    var metroTime = shortestTime - walkFromTime - walkToTime;
    return {
...
function shortestTransfer(graph, p1i, p2i) {
    var p1 = graph.platforms[p1i], p2 = graph.platforms[p2i];
    if (p1.station !== p2.station) {
        throw new Error("platforms (" + p1.name + " & " + p2.name + " must be on the same station");
    }
    var distanceBetweenPoints = function (a, b) { return L.LatLng.prototype.distanceTo.call(a, b); };
    var station = graph.stations[p1.station];
    ...
    while (platformSet.size > 0) {
        ...
        var platform = graph.platforms[currentIndex];
        var neighborIndices = platform.transfers;
        for (var _a = 0; _a < neighborIndices.length; _a++) {
            var neighborIndex = neighborIndices[_a];
            if (!platformSet.has(neighborIndex))
                continue;
            var distance = distanceBetweenPoints(platform.location, graph.platforms[neighborIndex].location), alt = dist[currentIndex] + distance;
    ...
    for (;;) {
        ...
        if (prevIndex === null)
            break;
        var transferIndex = graph.transfers.findIndex(function (t) { return t.source === currentIndex && t.target === prevIndex || t.source === prevIndex && t.target === currentIndex; });
...
},{"./geo":61,"./math":64,"./util":75}],60:[function(require,module,exports){
var svg = require('./svg');
var math = require('./math');
    ...
    props.forEach(function (prop, pi) {
        Object.defineProperty(transfer, prop, {
            ...
            set: function (platformIndex) {
                ...
                var circleTotalRadius = Number(circle.getAttribute('r')) / 2 + parseFloat(getComputedStyle(circle).strokeWidth);
                ...
                if (elements[0].tagName === 'line') {
                    ...
                    var gradient = document.getElementById("g-" + _this.graph.transfers.indexOf(transfer));
                    ...
                    var transfers = [];
                    var transferIndices = [];
                    for (var i = 0; i < _this.graph.transfers.length; ++i) {
                        var t = _this.graph.transfers[i];
                    ...
                    var cCenter = math.getCircumcenter(circumpoints);
...
function platformToModel(platform, circles) {
    ...
    var _a = typeof platform === 'number'
        ...
        : [this.graph.platforms.indexOf(platform), platform], idx = _a[0], obj = _a[1];
    ...
    Object.defineProperty(obj, 'location', {
        ...
        set: function (location) {
            ...
            var locForPos = _this.map.getZoom() < 12
                ? _this.graph.stations[obj.station].location
                : location;
            ...
            _this.whiskers[idx] = _this.makeWhiskers(idx);
            ...
            for (var _a = 0, _b = obj.spans; _a < _b.length; _a++) {
                ...
                _this.whiskers[neighborIndex] = _this.makeWhiskers(neighborIndex);
                _this.graph.platforms[neighborIndex].spans.forEach(function (si) { return spansToChange.add(si); });
            }
            spansToChange.forEach(function (spanIndex) {
                var span = _this.graph.spans[spanIndex];
                var srcN = span.source, trgN = span.target;
                var controlPoints = [_this.platformsOnSVG.get(srcN), _this.whiskers[srcN][spanIndex], _this.whiskers[trgN][spanIndex], _this.platformsOnSVG.get(trgN)];
            ...
            var oo = 0;
            for (var _c = 0, _d = _this.graph.transfers; _c < _d.length; _c++) {
                var tr = _d[_c];
                if (tr.source === idx) {
                    tr.source = idx;
                    ++oo;
...
},{"./math":64,"./svg":68}],61:[function(require,module,exports){
...
function findClosestObject(point, objects) {
    ...
    var closestDistance = point.distanceTo(closest.location);
    for (var i = 1, len = objects.length; i < len; ++i) {
        var tempDist = point.distanceTo(objects[i].location);
...
function findObjectsWithinRadius(point, objects, radius, sortArray) {
    ...
    var arr = objects.filter(function (obj) { return point.distanceTo(obj.location) <= radius; });
    if (sortArray) {
...
},{"leaflet":57}],62:[function(require,module,exports){
/// <reference path="../typings/tsd.d.ts" />
exports.userLanguage = (navigator.userLanguage || navigator.language).slice(0, 2).toLowerCase();
exports.dictionary = {};
function translate(text) {
    return text in exports.dictionary && exports.userLanguage in exports.dictionary[text] ? exports.dictionary[text][exports.userLanguage] : text;
}
...
polyfills_1.default();
res.dictionaryPromise.then(function (dict) {
    var metroMap = new metro_map_1.default('map-container', 'json/graph.json');
 ...
 */ /**/ 
},{"./metro-map":65,"./polyfills":66,"./res":67,"./util":75,"leaflet":57}],64:[function(require,module,exports){
function dot(v1, v2) {
...
var minZoom = 9, maxZoom = 18;
var MetroMap = (function () {
    function MetroMap(containerId, kml) {
        var _this = this;
        this.whiskers = [];
        ...
        var graphPromise = fetch(kml)
            ...
            .then(function (graphJSON) { return _this.graph = graphJSON; });
        ...
        addEventListener('keydown', function (e) {
            if (!e.ctrlKey)
                return;
            switch (e.keyCode) {
                case 76: return tilelayers_1.mapnik.addTo(_this.map).bringToFront();
                default: return;
            }
        });
        this.overlay = document.getElementById('overlay');
        var container = this.map.getContainer();
        var objectsPane = this.map.getPanes().objectsPane;
        ...
        if (defs.textContent.length === 0) {
            alert(lang_1.translate("Your browser doesn't seem to have capabilities to display some features of the map. Consider using Chrome or Firefox for the best experience."));
        }
        this.overlay.appendChild(defs);
        graphPromise
            .catch(function (errText) { return alert(errText); })
            .then(function (graphJson) { return hintsPromise; })
            .then(function (hintsJson) { return res_1.lineRulesPromise; })
            .then(function (lineRules) {
            _this.lineRules = lineRules;
            _this.bounds = new L.LatLngBounds(_this.graph.platforms.map(function (p) { return p.location; }));
            _this.redrawNetwork();
            ...
            //util.drawZones(this);
            if (!L.Browser.mobile) {
                new ui_1.MapEditor(_this);
            }
            new ui_1.FAQ(_this, 'json/data.json');
            return contextMenuPromise;
        })
            .then(function (contextMenuData) {
            _this._contextMenu = new ui_1.ContextMenu(_this, new Map(contextMenuData));
            _this.resetMapView();
            util.loadIcons(_this.map, [_this.fromMarker, _this.toMarker]);
            util.fixFontRendering(); // just in case
        });
        Promise.all([graphPromise, hintsPromise])
            .then(function (results) { return util.Hints.verify(_this.graph, _this.hints); })
        ...
        switch (event.type) {
            ...
            case 'showheatmap':
                if (this.map.hasLayer(this.fromMarker)) {
                    ...
                    var topLeft = this.map.latLngToContainerPoint(nw);
                    var se = this.bounds.getSouthEast();
                    var bottomRight = this.map.latLngToContainerPoint(se);
                    var step = bottomRight.subtract(topLeft).divideBy(200);
                    console.log(se.distanceTo(nw) / 200);
                    console.log('for ' + step);
                    console.time('checking');
                    for (var i = topLeft.x; i < bottomRight.x; i += step.x) {
                        for (var j = topLeft.y; j < bottomRight.y; j += step.y) {
                            var c = this.map.containerPointToLatLng(new L.Point(i, j));
                            //new L.Marker(c).addTo(this.map);
                            var closest = geo.findClosestObject(c, this.graph.platforms).location;
            ...
            case 'measuredistance': {
                var coors = util.mouseToLatLng(this.map, event);
                ...
                // break;
                var ptGroup = document.getElementById('measure-circles');
                var lineGroup = document.getElementById('measure-lines');
                if (ptGroup !== null) {
                    origin.removeChild(ptGroup);
                    origin.removeChild(lineGroup);
                }
                ptGroup = svg.createSVGElement('g');
                ptGroup.id = 'measure-circles';
                lineGroup = svg.createSVGElement('g');
                ...
                this.map.on('click', function (e) {
                    if (e.originalEvent.button !== 0)
                        return;
                    var pos = _this.map.latLngToContainerPoint(e.latlng).subtract(_this.map.latLngToContainerPoint(_this.bounds.getNorthWest()));
                    var circle = svg.makeCircle(pos, 3);
                    circle.classList.add('measure-circle');
                    var el = { lang: { ru: 'Udaliť izmerenia', en: 'Delete measurements' } };
                    _this._contextMenu.extraItems.set(circle, new Map().set('deletemeasurements', el));
                    ptGroup.appendChild(circle);
                    var prevPos = _this.map.latLngToContainerPoint(prevCoors)
                        .subtract(_this.map.latLngToContainerPoint(_this.bounds.getNorthWest()));
                    var line = svg.makeLine(prevPos, pos);
                ...
                this.map.fire('click', { latlng: coors, originalEvent: { button: 0 } });
                util.onceEscapePress(function (e) { return _this.dispatchEvent(new MouseEvent('deletemeasurements')); });
                break;
            }
            case 'deletemeasurements': {
                var origin = document.getElementById('origin');
                for (var _i = 0, _a = ['measure-circles', 'measure-lines']; _i < _a.length; _i++) {
                    var id_1 = _a[_i];
                    origin.removeChild(document.getElementById(id_1));
                }
                this.map.off('click');
                break;
            }
            case 'platformrename': {
                var me = event;
                var platform_1 = svg.platformByCircle(me.relatedTarget, this.graph);
                this.plate.show(svg.circleByDummy(me.relatedTarget));
                util.platformRenameDialog(this.graph, platform_1);
                break;
            }
            case 'platformdelete': {
            }
            case 'platformadd':
                ...
                var id = this.graph.platforms.length;
                circle.id = 'p-' + id;
                dummy.id = 'd-' + id;
                stationCircles.appendChild(circle);
                dummyCircles.appendChild(dummy);
                var platform = {
                    name: lang_1.translate('New station'),
                    altNames: {},
                    station: null,
                    spans: [],
                    transfers: [],
    ...
    MetroMap.prototype.handleMenuFromTo = function (e) {
        ...
        var marker = e.type === 'fromclick' ? this.fromMarker : this.toMarker;
        ...
        if (this.map.hasLayer(this.fromMarker) && this.map.hasLayer(this.toMarker)) {
            ...
            // }
            var onMarkerDrag = function (e) { return _this.visualizeShortestRoute([_this.fromMarker.getLatLng(), _this.toMarker.getLatLng()], false); };
            var onMarkerDragEnd = function (e) {
                util.fixFontRendering();
                _this.visualizeShortestRoute([_this.fromMarker.getLatLng(), _this.toMarker.getLatLng()]);
            };
        ...
        util.onceEscapePress(function (e) { return _this.dispatchEvent(new MouseEvent('clearroute')); });
    };
    MetroMap.prototype.addMapListeners = function () {
        ...
        this.map.on('movestart', function (e) {
            ...
            var mousePos = e.target['scrollWheelZoom']['_lastMousePos'];
            var fromZoom = e.target['_zoom'];
            if (scaleFactor === 1) {
                setTimeout(function () {
                    var toZoom = e.target['_animateToZoom'];
                    scaleFactor = Math.pow(2, (toZoom - fromZoom));
        ...
        }).on('layeradd', function (e) { return util.fixFontRendering(); });
        this.overlay.addEventListener('wheel', function (e) { return scaleFactor *= e.deltaY < 0 ? 2 : 0.5; });
    };
    ...
    MetroMap.prototype.resetOverlayStructure = function () {
        var origin = document.getElementById('origin') || svg.createSVGElement('g');
        ...
        this.plate = new ui_1.TextPlate(this.graph);
        origin.insertBefore(this.plate.element, document.getElementById('dummy-circles'));
    };
    MetroMap.prototype.addBindings = function () {
        var graphPlatforms = this.graph.platforms;
        for (var i = 0; i < graphPlatforms.length; ++i) {
    ...
    MetroMap.prototype.redrawNetwork = function () {
        ...
        var lineWidth = (zoom - 7) * 0.5; // or just (zoom - 9) ? 
        ...
        for (var _b = 0, _c = this.graph.stations; _b < _c.length; _b++) {
            var station = _c[_b];
            var circumpoints = [];
            for (var _d = 0, _e = station.platforms; _d < _e.length; _d++) {
                ...
                if (zoom > 9) {
                    ...
                    if (zoom > 11) {
                        var lines = this.passingLines(platform);
                        if (lines.size === 1) {
                            var tokens = lines.values().next().value.match(/([MEL])(\d{0,2})/);
                            if (tokens) {
                            ...
                            for (var vals = lines.values(), it = vals.next(); !it.done; it = vals.next()) {
                                var tokens = it.value.match(/([MEL])(\d{0,2})/);
                                if (!tokens)
                                    return;
            ...
            dummyCircles.addEventListener('mouseover', function (e) { return _this.plate.show(svg.circleByDummy(e.target)); });
            dummyCircles.addEventListener('mouseout', function (e) { return _this.plate.hide(); });
            var circular = algorithm.findCircle(this.graph, station);
        ...
        if (zoom > 11) {
            ...
            for (var transferIndex = 0; transferIndex < this.graph.transfers.length; ++transferIndex) {
                var transfer = this.graph.transfers[transferIndex];
                var paths;
                var pl1 = this.graph.platforms[transfer.source], pl2 = this.graph.platforms[transfer.target];
                ...
                if (platformsInCircles.has(transfer.source) && platformsInCircles.has(transfer.target)) {
                    var cluster = stationCircumpoints.get(this.graph.stations[pl1.station]);
                    var makeArc = function (third) {
                        var thirdPos = _this.platformsOnSVG.get(_this.graph.platforms.indexOf(third));
                        paths = svg.makeTransferArc(pos1, pos2, thirdPos);
                    };
                    if (cluster.length === 3) {
                        makeArc(cluster.find(function (p) { return p !== pl1 && p !== pl2; }));
                    }
                    else if (pl1 === cluster[2] && pl2 === cluster[3] || pl1 === cluster[3] && pl2 === cluster[2]) {
                        paths = svg.makeTransfer(pos1, pos2);
                    }
                    else {
                        var degs = cluster.map(function (p) { return p.transfers.length; });
                        var _h = pl1.transfers.length === 2 ? [pl1, pl2] : [pl2, pl1], a = _h[0], b = _h[1];
                        makeArc(this.graph.platforms[a.transfers.find(function (i) { return _this.graph.platforms[i] !== b; })]);
                    }
                }
                else {
                    paths = svg.makeTransfer(pos1, pos2);
                ...
                var gradientColors = [pl1, pl2].map(function (p) {
                    var span = _this.graph.spans[p.spans[0]];
                    var routes = span.routes.map(function (n) { return _this.graph.routes[n]; });
                    var _a = routes[0].line.match(/([MEL])(\d{0,2})/), lineId = _a[0], lineType = _a[1], lineNum = _a[2];
                    return _this.lineRules.get(lineType === 'L' ? lineType : lineId);
                });
                ...
                var circlePortion = (circleRadius + circleBorder / 2) / pos1.distanceTo(pos2);
        ...
        for (var i = 0; i < this.graph.spans.length; ++i) {
            var _j = this.makePath(i), outer = _j[0], inner = _j[1];
    ...
    MetroMap.prototype.getPlatformsPositionOnOverlay = function () {
        var zoom = this.map.getZoom();
        var nw = this.bounds.getNorthWest();
        var se = this.bounds.getSouthEast();
        var svgBounds = new L.Bounds(this.map.latLngToContainerPoint(nw), this.map.latLngToContainerPoint(se));
        var graphPlatforms = this.graph.platforms;
        var stationCenter = function (st) { return geo.getCenter(st.platforms.map(function (i) { return graphPlatforms[i].location; })); };
        for (var _i = 0, _a = this.graph.stations; _i < _a.length; _i++) {
            var station = _a[_i];
            var center = zoom < 12 ? stationCenter(station) : null;
            for (var _b = 0, _c = station.platforms; _b < _c.length; _b++) {
                var platformIndex = _c[_b];
                var pos = this.posOnSVG(svgBounds, center || graphPlatforms[platformIndex].location);
                this.platformsOnSVG.set(platformIndex, pos);
            }
        }
    };
    MetroMap.prototype.passingLines = function (platform) {
        var lines = new Set();
        for (var _i = 0, _a = platform.spans; _i < _a.length; _i++) {
            var spanIndex = _a[_i];
            for (var _b = 0, _c = this.graph.spans[spanIndex].routes; _b < _c.length; _b++) {
    ...
    MetroMap.prototype.makeWhiskers = function (platformIndex) {
        ...
        if (platform.spans.length > 2) {
            ...
            var idx = util.Hints.hintContainsLine(this.graph, dirHints, platform);
            ...
            var whisker = {};
            spanIds[0].forEach(function (spanIndex) { return whisker[spanIndex] = midPts_1[0].add(diff_1); });
            spanIds[1].forEach(function (spanIndex) { return whisker[spanIndex] = midPts_1[1].add(diff_1); });
            return whisker;
        }
        var lines = platform.spans.map(function (i) { return _this.graph.routes[_this.graph.spans[i].routes[0]].line; });
        // TODO: refactor this stuff, unify 2-span & >2-span platforms
        if (lines[0] !== lines[1]) {
            return (_e = {},
                _e[platform.spans[0]] = posOnSVG,
                _e[platform.spans[1]] = posOnSVG,
                _e
            );
        }
        ...
        for (var i = 0; i < 2; ++i) {
            ...
            var neighborOnSVG = this.platformsOnSVG.get(neighborNum);
        ...
        return (_f = {},
            _f[platform.spans[0]] = midPts[0].add(diff),
            _f[platform.spans[1]] = midPts[1].add(diff),
            _f
        );
        var _a, _e, _f;
    };
    ...
    MetroMap.prototype.visualizeShortestRoute = function (obj, animate) {
        var _this = this;
        if (animate === void 0) { animate = true; }
        util.resetStyle();
        alertify.dismissAll();
        var o = obj instanceof Array ? algorithm.shortestRoute(this.graph, obj[0], obj[1]) : obj;
        var _a = o, platforms = _a.platforms, edges = _a.edges, time = _a.time;
        var onFoot = lang_1.translate('on foot');
        var walkTo = lang_1.formatTime(time.walkTo);
        if (edges === undefined) {
        ...
        svg.Animation.terminateAnimations().then(function () {
            var els = _this.overlay.querySelectorAll(selector);
            for (var i = 0; i < els.length; ++i) {
                var elStyle = els[i].style;
            ...
            for (var _a = 0; _a < platforms.length; _a++) {
                var platformNum = platforms[_a];
                document.getElementById('p-' + platformNum).style.opacity = null;
            }
        }).catch(function (animate) { return svg.Animation.animateRoute(_this.graph, platforms, edges); }).then(function (finished) {
            if (!finished)
                return;
            alertify.message(lang_1.translate('time').toUpperCase() + ":<br>" + walkTo + " " + onFoot + "<br>" + lang_1.formatTime(time.metro) + " " + lang_1.translate('by metro') + "<br>" + lang_1.formatTime(time.walkFrom) + " " + onFoot + "<br>" + lang_1.translate('TOTAL') + ": " + lang_1.formatTime(time.total), 10);
        });
    };
    return MetroMap;
})();
...
},{"./algorithm":59,"./bind":60,"./geo":61,"./lang":62,"./math":64,"./res":67,"./svg":68,"./tilelayers":69,"./ui":70,"./util":75,"alertifyjs":1,"leaflet":57}],66:[function(require,module,exports){
function default_1() {
...
},{"classlist-polyfill":2,"es6-promise":44,"es6-set/implement":45}],67:[function(require,module,exports){
var lang = require('./lang');
exports.dictionaryPromise = fetch('json/dictionary.json')
    .then(function (data) { return data.json(); })
    .then(function (dict) { return lang.dictionary = dict; })
...
},{"./lang":62}],68:[function(require,module,exports){
var L = require('leaflet');
var math = require('./math');
var util_1 = require('./util');
...
function setCircularPath(el, start, end, third) {
    ...
    var startAngle = Math.atan2(start.y - center.y, start.x - center.x), endAngle = Math.atan2(end.y - center.y, end.x - center.x), thirdAngle = Math.atan2(third.y - center.y, third.x - center.x);
...
(function (Shadows) {
    ...
    function applyDrop(path) {
        ...
        var strokeWidth = parseFloat(getComputedStyle(path).strokeWidth);
...
(function (Gradients) {
    ...
    function removeAll() {
        ...
        // }
        var transfers = document.getElementById('transfers-outer').children;
        if (transfers) {
            for (var i = 0, len = transfers.length; i < len; ++i) {
                var transfer = transfers[i];
                var fallbackColor = transfer.getAttribute('data-fallbackcolor');
                if (fallbackColor === null) {
                    var gradient = document.getElementById('g-' + i);
                    var colors = [gradient.firstElementChild, gradient.lastElementChild]
                        .map(function (el) { return el.style.stopColor; });
                    fallbackColor = util_1.Color.mean(colors);
...
(function (Animation) {
    ...
    function animateRoute(graph, platforms, edges) {
        currentAnimation = new Promise(function (resolve, reject) { return (function animateSpan(i) {
            ...
            var edge = graph[idParts[0] === 'p' ? 'spans' : 'transfers'][+idParts[1]];
    ...
    function pulsateCircle(circle, scaleFactor, duration) {
        ...
        var t = scaleFactor - 1, tx = -circle.getAttribute('cx') * t, ty = -circle.getAttribute('cy') * t;
...
},{"./math":64,"./util":75,"leaflet":57}],69:[function(require,module,exports){
var L = require('leaflet');
...
exports.openMapSurfer = new L.TileLayer('http://openmapsurfer.uni-hd.de/tiles/roads/x={x}&y={y}&z={z}', {
    minZoom: 9,
    detectRetina: true,
    //reuseTiles: true,
...
exports.hyddaBase = L.tileLayer('http://{s}.tile.openstreetmap.se/hydda/base/{z}/{x}/{y}.png', {
    minZoom: 9,
    detectRetina: true,
...
exports.esriGrey = L.tileLayer('http://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}', {
    attribution: 'Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ',
    minZoom: 9,
    detectRetina: true,
});
},{"leaflet":57}],70:[function(require,module,exports){
...
exports.Icons = Icons;
},{"./ui/contextmenu":71,"./ui/faq":72,"./ui/mapeditor":73,"./ui/textplate":74}],71:[function(require,module,exports){
var L = require('leaflet');
var lang = require('../lang');
var ContextMenu = (function () {
    function ContextMenu(metroMap, items) {
        ...
        var listener = this.handler.bind(this);
...
},{"../lang":62,"leaflet":57}],72:[function(require,module,exports){
var Hammer = require('hammerjs');
var util_1 = require('../util');
var FAQ = (function () {
    function FAQ(map, faqDataUrl) {
        var _this = this;
        var promise = fetch(faqDataUrl)
            .catch(function (err) { return console.error(err); })
            .then(function (data) { return data.json(); });
        this.map = map.getMap();
        ...
        promise.then(function (data) { return _this.card.innerHTML += data.faq.map(qa2html).join(''); });
    }
    FAQ.prototype.showFAQ = function (event) {
        var _this = this;
        ...
        if (!L.Browser.mobile) {
            ...
            this.map.once('mousedown', function (e) { return _this.hideFAQ(e.originalEvent); });
    ...
    FAQ.prototype.hideFAQ = function (event) {
        var _this = this;
        console.log('hiding');
        this.card.getBoundingClientRect();
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = FAQ;
},{"../util":75,"hammerjs":56}],73:[function(require,module,exports){
var util_1 = require('../util');
var svg = require('../svg');
var lang = require('../lang');
var MapEditor = (function () {
    function MapEditor(metroMap) {
        var _this = this;
        this.metroMap = metroMap;
        var btn = document.createElement('button');
    ...
    MapEditor.prototype.editMapClick = function (event) {
        this.editMode = true;
    ...
    MapEditor.prototype.saveMapClick = function (event) {
        var graph = this.metroMap.getGraph();
        var content = JSON.stringify(graph, function (key, val) { return key.startsWith('_') ? undefined : val; });
        util_1.downloadAsFile('graph.json', content);
        this.editMode = false;
    };
    MapEditor.prototype.addMapListeners = function () {
        ...
        dummyCircles.onmousedown = function (de) {
            if (de.button === 0) {
                var platform = svg.platformByCircle(de.target, graph);
                ...
                map.once('mouseup', function (le) {
                    ...
                    plate.show(svg.circleByDummy(le.originalEvent.target));
                });
            }
            else if (de.button === 1) {
            }
...
},{"../lang":62,"../svg":68,"../util":75}],74:[function(require,module,exports){
var svg = require('../svg');
var lang = require('../lang');
var TextPlate = (function () {
    function TextPlate(graph) {
        ...
        var createElement = function (tag, id, cls, attributes) {
            var el = svg.createSVGElement(tag);
            el.id = id;
            el.classList.add(cls);
            Object.keys(attributes).forEach(function (key) { return el.setAttribute(key, attributes[key]); });
            return el;
        };
        var pole = createElement('line', 'pole', 'plate-pole', { x1: 0, y1: 0, x2: 4, y2: 8 });
        this._element.appendChild(pole);
        var g = svg.createSVGElement('g');
        var rect = createElement('rect', 'plate-box', 'plate-box', { x: 0, y: 0, filter: 'url(#shadow)' });
        g.appendChild(rect);
        var text = createElement('text', 'plate-text', 'plate-text', { fill: 'black', x: 0, y: 0 });
        var tspan = svg.createSVGElement('tspan');
    ...
    TextPlate.prototype.show = function (circle) {
        if (this.disabled)
            return;
    ...
    TextPlate.prototype.modify = function (circle) {
        var c = new L.Point(Number(circle.getAttribute('cx')), Number(circle.getAttribute('cy')));
        var r = Number(circle.getAttribute('r'));
        var iR = Math.trunc(r);
        var pole = this._element.firstElementChild;
        var poleSize = new L.Point(4 + iR, 8 + iR);
        var platform = svg.platformByCircle(circle, this.graph);
        var ru = platform.name;
        var fi = platform.altNames['fi'];
        var en = platform.altNames['en'];
        var names = !fi ? [ru] : lang.userLanguage === 'fi' ? [fi, ru] : [ru, fi];
        if (en)
            names.push(en);
        this.modifyBox(c.subtract(poleSize), names);
    };
    TextPlate.prototype.modifyBox = function (bottomRight, lines) {
        var _this = this;
        var g = this._element.lastChild;
        ...
        var adjustDimensions = function (y) {
            var _a = text.getBBox(), width = _a.width, height = _a.height;
            width = Math.round(width);
            rect.setAttribute('width', (width + 6).toString());
            rect.setAttribute('height', (height + y).toString());
            var tx = bottomRight.x - width, ty = bottomRight.y - height;
            _this._element.setAttribute('transform', "translate(" + tx + ", " + ty + ")");
...
//    return <any>foreign;
//}
},{"../lang":62,"../svg":68}],75:[function(require,module,exports){
/// <reference path="../typings/tsd.d.ts" />
var L = require('leaflet');
var lang = require('./lang');
var tr = function (text) { return lang.translate(text); };
var alertify = require('alertifyjs');
...
function mouseToLatLng(map, event) {
    var clientPos = new L.Point(event.clientX, event.clientY);
    var rect = map.getContainer().getBoundingClientRect();
    var containerPos = new L.Point(rect.left, rect.top);
    var coors = map.containerPointToLatLng(clientPos.subtract(containerPos));
    return coors;
}
...
var Hints;
(function (Hints) {
    function verify(graph, hints) {
        function checkExistence(val) {
            if (graph.platforms.find(function (el) { return el.name === val; }) === undefined) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            Object.keys(crossPlatform).forEach(function (platformName) {
                if (graph.platforms.find(function (el) { return el.name === platformName; }) === undefined) {
    ...
    function hintContainsLine(graph, dirHints, platform) {
        var spans = platform.spans.map(function (i) { return graph.spans[i]; });
        var routes = [];
        spans.forEach(function (span) { return span.routes.forEach(function (i) { return routes.push(graph.routes[i]); }); });
    ...
    Hints.hintContainsLine = hintContainsLine;
})(Hints = exports.Hints || (exports.Hints = {}));
var Color;
(function (Color) {
...
function platformRenameDialog(graph, platform) {
    ...
    alertify.prompt(tr('New name'), nameString, function (okevt, val) {
        ...
        var station = graph.stations[platform.station];
        if (station.platforms.length < 2)
            return;
        alertify.confirm(tr('Rename the entire station') + '?', function () {
            for (var _i = 0, _a = station.platforms; _i < _a.length; _i++) {
                var i = _a[_i];
                var p = graph.platforms[i];
    ...
    }, function () { return alertify.warning(tr('Name change cancelled')); });
}
...
function loadIcons(map, markers) {
...
function fixFontRendering() {
    var blurringStuff = document.querySelectorAll('[style*="translate3d"]');
...
},{"./geo":61,"./lang":62,"alertifyjs":1,"leaflet":57}],76:[function(require,module,exports){
