...
function shortestRoute(graph, p1, p2) {
    var walkingSpeed = 1.4, walkingWithObstacles = 1, maxTrainSpeed = 20, trainAcceleration = 0.7, metroStopTime = 30, eLineStopTime = 60, 
    // includes escalators
 ...
 */ /**/ 
},{"./metro-map":65,"./polyfills":66,"./res":67,"./tilelayers":69,"./util":75,"leaflet":57}],64:[function(require,module,exports){
function dot(v1, v2) {
...
var MetroMap = (function () {
    function MetroMap(containerId, kml, tileLayers) {
        ...
        this.fromMarker = new L.Marker([0, 0]);
        this.toMarker = new L.Marker([0, 0]);
        ...
        graphPromise
            ...
            .then(function (graphJson) { return _this.extendBounds(); }) // because the previous assignment returns json
            .then(function () { return hintsPromise; })
            ...
            .then(function (contextMenuData) { return _this._contextMenu = new ui_1.ContextMenu(_this, new Map(contextMenuData)); });
    ...
    MetroMap.prototype.dispatchEvent = function (event) {
        ...
        switch (event.type) {
            ...
            case 'platformdelete': {
                var me = event;
                var platform_2 = svg.platformByCircle(me.relatedTarget, this.graph);
                var spans = platform_2.spans;
                var pIdx = this.graph.platforms.indexOf(platform_2);
                var lines = this.passingLines(platform_2);
                if (lines.size === 1) {
                    var spans_1 = this.graph.spans.filter(function (s) { return s.source === pIdx || s.target === pIdx; });
                    if (spans_1.length === 1) {
                    }
                    else if (spans_1.length === 2) {
                        // delete the second one
                        var a = spans_1[0], b = spans_1[1];
                        a[a.source === pIdx ? 'target' : 'source'] = b[a.source === pIdx ? 'target' : 'source'];
                        var sIdx = this.graph.spans.indexOf(b);
                        this.graph.spans.splice(sIdx, 1);
                        for (var _b = 0, _c = this.graph.platforms; _b < _c.length; _b++) {
                            var p = _c[_b];
                            var that = null;
                ...
                for (var _d = 0, _e = this.graph.spans; _d < _e.length; _d++) {
                    var s = _e[_d];
                    if (s.source > pIdx) {
                        --s.source;
                    }
                ...
                for (var _f = 0, _g = this.graph.stations; _f < _g.length; _f++) {
                    var s = _g[_f];
                    var that = null;
    ...
    MetroMap.prototype.addMapListeners = function () {
        ...
        this.map.on('movestart', function (e) {
            ...
            //this.overlay.style.transform = mapPane.style.transform;
            _this.overlay.style.transform = null;
        }).on('zoomstart', function (e) {
            console.log('zoomstart', e);
            var a = _this.map.getPanes().mapPane;
            //a.style.transition = null;
            _this.map.dragging.disable();
            var fromZoom = e.target['_zoom'];
            setTimeout(function () {
                var toZoom = e.target['_animateToZoom'];
                var scaleFactor = Math.pow(2, (toZoom - fromZoom));
                var box = _this.overlay.getBoundingClientRect();
                var client = e.target['scrollWheelZoom']['_lastMousePos'];
                if (!client) {
                    var el = document.documentElement;
                    client = new L.Point(el.clientWidth / 2, el.clientHeight / 2);
                }
                var clickOffset = new L.Point(client.x - box.left, client.y - box.top);
                var ratio = new L.Point(clickOffset.x / box.width, clickOffset.y / box.height);
                var overlayStyle = _this.overlay.style;
                // overlayStyle.left = '0';
                // overlayStyle.top = '0';
            ...
            _this.overlay.style.transformOrigin = null;
            _this.redrawNetwork();
     ...
     */
    MetroMap.prototype.fixFontRendering = function () {
        var blurringStuff = document.querySelectorAll('img[style*="translate3d"]');
    ...
    MetroMap.prototype.redrawNetwork = function () {
        ...
        for (var _k = 0, _l = Object.keys(docFrags); _k < _l.length; _k++) {
            var i = _l[_k];
    ...
    MetroMap.prototype.visualizeShortestRoute = function (departure, arrival) {
        var _this = this;
        util.resetStyle();
        alertify.dismissAll();
        var _a = algorithm.shortestRoute(this.graph, departure, arrival), platforms = _a.platforms, edges = _a.edges, time = _a.time;
        ...
        var els = this.overlay.querySelectorAll(selector);
        for (var i = 0; i < els.length; ++i) {
            //els[i].style['-webkit-filter'] = 'grayscale(1)';
            els[i].style.opacity = '0.25';
        }
        console.log(edges);
        console.log(platforms.map(function (p) { return _this.graph.platforms[p].name; }));
        svg.Animation.animateRoute(this.graph, platforms, edges).then(function () {
...
},{"./algorithm":59,"./bind":60,"./geo":61,"./lang":62,"./math":64,"./res":67,"./svg":68,"./ui":70,"./util":75,"alertifyjs":1,"leaflet":57}],66:[function(require,module,exports){
function default_1() {
...
(function (Animation) {
    function animateRoute(graph, platforms, edges) {
        return new Promise(function (resolve) { return (function animateSpan(i) {
            ...
            var length = outer instanceof SVGLineElement
            ...
            outer.addEventListener('transitionend', function (e) {
                ...
                if (outer.id.charAt(1) !== 't') {
                    ...
                    var strokeWidth = parseFloat(getComputedStyle(outerOld).strokeWidth);
...
var ContextMenu = (function () {
    ...
    ContextMenu.prototype.handler = function (event) {
        ...
        this.table.onclick = function (e) {
            ...
            if (eventType) {
                _this.state = false;
                _this.metroMap.dispatchEvent(new MouseEvent(eventType, dict));
            }
        };
...
(function (CSSTransform) {
    ...
    CSSTransform.parse = parse;
    function replace(el) {
        var s = el.style;
        var re = /translate3d\s*\((.+?,\s*.+?),\s*.+?\s*\)/i;
        s.transform = s.transform.replace(re, 'translate($1)');
    }
    CSSTransform.replace = replace;
...
function drawZones(metroMap) {
    ...
    var fitnessFunc = function (points, pt) { return points.reduce(function (prev, cur) { return prev + pt.distanceTo(cur); }, 0); };
    var poly = L.polyline([]);
